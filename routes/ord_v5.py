...
def _validate_ord_header(ord_: dict, tip: str):
    ...
    # _opt_int converteste 0 → None, deci Access poate trimite 0 sau null
...
def _validate_tbl_rec(r: dict, idx: int, valid_tbl_tmpid_set: set):
    ...
    _strict_pos_int(r.get("IDR"),   f"{pfx}.IDR") 
...
def _validate_payload(data: dict, tip: str):
    ...
    Ordinea: header → limits → parts → valid_tmpid_set → tbls/atts/docs.
...
# Apelate in ordinea: Ord → Parts → Tbls → Atts → Docs (ordinea impusa
# de FK-urile din staging: stg_OrdPart.Token → stg_Ord.Token etc.)
...
def _stg_insert_all(cursor, token: str, tip: str, data: dict):
    """
    Insereaza complet un payload in staging (toate cele 5 tabele).
    Ordinea este impusa de FK: Ord → Parts → Tbls → Atts → Docs.
...
# pentru toate entitatile (Part_Map, TBL_Map, ATT_Map, DOC_Map).
...
def _commit_add(cursor, token: str) -> dict:
    """
    Commit ADD complet: INSERT FX_ORD → PART → TBL → ATT → DOC.
...
def _commit_mod(cursor, token: str) -> dict:
    """
    Commit MOD complet: UPDATE FX_ORD → diff sync PART → TBL → ATT → DOC.
...
def confirm():
    ...
    status="OK":
      ...
      - Returneaza {IDORDP, Part_Map, TBL_Map, ATT_Map, DOC_Map}.
...
def delete_ord():
    ...
    try:
        ...
        def operation(cursor):
            ...
            if cursor.rowcount == 0:
                raise ValueError(
                    f"DELETE FX_ORD IDORDP={idordp}: 0 randuri afectate"
                )
